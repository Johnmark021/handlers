...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["submitBtn"])) {
    ...
    if (!filter_var($_POST["email"], FILTER_VALIDATE_EMAIL)) {
        $errors_ .= Util::displayAlertV1("Please enters a valid email address", "warning");
    }
    ...
    if (!empty($errors_)) {
        ...
        if ($handler->isCustomerExists($_POST['email'])==0) {
            echo Util::displayAlertV1("Incorrect email.", "warning");
            return;
        }
